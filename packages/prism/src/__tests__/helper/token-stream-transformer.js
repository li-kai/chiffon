...
function prettyFormat(prettyStream) {
  ...
  const ranges = prettySplit(
    prettyStream,
    (br) => br instanceof LineBreakItem && br.enabled,
  )
        ...
        return lines.map((g) => {
          if (g.length > 1) {
            return (
              g
                .map((item) =>
                  isToken(item) ? ', '.length + JSON.stringify(item).length : 0,
                )
...
function prettySomeLineBreak(prettyStream, recursive, cond) {
  for (const item of prettyStream) {
    if (item instanceof LineBreakItem && cond(item)) {
      ...
      recursive &&
      isNested(item) &&
      prettySomeLineBreak(item[1], true, cond)
    ) {
      return true
    }
  }
...
function isTriviallyNested(item) {
  return (
    isNested(item) && item[1].length === 1 && typeof item[1][0] === 'string'
  )
}
