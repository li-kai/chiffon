...
const TestCase = {
  ...
  async runTestCase(languageIdentifier, filePath, acceptEmpty) {
    ...
    if (testCase.expectedTokenStream === null) {
      ...
      if (!acceptEmpty) {
        throw new Error(
          "This test case doesn't have an expected toke n stream." +
      ...
      const separator =
        '\n\n----------------------------------------------------\n\n'
      const pretty = TokenStreamTransformer.prettyprint(tokenStream).replace(
        /^( +)/gm,
        (m) => {
      ...
      const diffIndex = translateIndexIgnoreSpaces(
        testCase.expectedJson,
        expected,
        firstDiff(expected, actual),
      )
      const expectedJsonLines = testCase.expectedJson
        .substr(0, diffIndex)
        .split(/\r\n?|\n/g)
