...
module.exports = (Prism) => {
  ...
  function extendTest(id, redef) {
    ...
    if (!(id in Prism.languages)) {
      throw new Error(
        `Cannot extend '${id}' because it is not defined in Prism.languages.`,
      )
    }
    ...
    if ('rest' in redef) {
      throw new Error(
        `The redef object is not allowed to contain a "rest" property.` +
          details,
      )
    }
    ...
    if (isReference) {
      throw new Error(
        `The redef object cannot be a reference to an existing language.` +
    ...
    if (!redef.__cloned) {
      ...
      const overwriteKeys = redefKeys.filter(
        (k) => languageKeys.indexOf(k) !== -1,
      )
      ...
      if (!arrayEqual(redefKeys, [...overwriteKeys, ...appendKeys])) {
        ...
        const offenders = appendKeys.filter(
          (k) => redefKeys.indexOf(k) < lastOverwriteIndex,
        )
        ...
        throw new Error(
          `All tokens in the redef object that do not overwrite tokens of the extended language have to be placed after the last token that does overwrite an existing token.` +
            ` Move the token(s) ${offendersList} after the '${lastOverwrite}' token.` +
            details,
        )
      }
      ...
      if (!arrayEqual(overwriteKeys, sortedOverwriteKeys)) {
        const trimmedUnsorted = trimCommon(overwriteKeys, sortedOverwriteKeys)
          .map((k) => `'${k}'`)
          .join(', ')
        const trimmedSorted = trimCommon(sortedOverwriteKeys, overwriteKeys)
          .map((k) => `'${k}'`)
          .join(', ')
        throw new Error(
          'The tokens in the redef object that overwrite existing tokens of the extended language should be in the order of the existing tokens. ' +
            `The tokens ${trimmedUnsorted} should be in the order ${trimmedSorted}.` +
            details,
        )
      }
    }
  ...
  function insertBeforeTest(inside, before, insert, root) {
    ...
    if (typeof insert !== 'object') {
      throw new TypeError(
        `The insert argument has to be an 'object'.` + details,
      )
    }
    if (root && typeof root !== 'object') {
      throw new TypeError(
        `The root argument has to be an 'object' if defined.` + details,
      )
    }
    ...
    if (typeof grammar !== 'object') {
      throw new Error(
        `The grammar "${inside}" has to be an 'object' not '${typeof grammar}'.`,
      )
    }
